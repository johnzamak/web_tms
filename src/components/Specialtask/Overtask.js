...
class Overtask extends Component {
    constructor(props){
        super(props)
        this.state={
    ...
    get_tsc_today(self) {
        ...
        let url = proxy.main + "special-circles/get-over-task/" + date_start + "&" + date_end
        fetch(url)
            .then(response => response.json())
            .then((responseJson) => {
                console.log("responseJson",responseJson)
                let arr_data=[],mdStatus="",qrcode_value=""
                if(responseJson.status===200){
                    responseJson.result.forEach((val,i) => {
                        qrcode_value="::"+val.tsc_document
                        switch(val.status){
                            case 0:mdStatus=<MdIcon.MdUpdate className="iconlg" />;break;
                            case 3:mdStatus=<MdIcon.MdMoveToInbox className="iconlg" />;break;
                            case 4:mdStatus=<MdIcon.MdLocalShipping className="iconlg" />;break;
                            case 10:mdStatus=<MdIcon.MdCheckCircle className="iconlg" />;break;
                        }
                        arr_data.push(
                            <tr>
