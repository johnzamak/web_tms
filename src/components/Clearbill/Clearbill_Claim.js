...
class Clearbill_Claim extends Component {
    constructor(props) {
        super(props)
        this.state = {
            show_date_bill: "",
    ...
    get_data_from_api(self) {
        ...
        var url = proxy.main + "clearbill/clearbill-mess-claim/" + messCode + "&" + inDate
        fetch(url)
            .then(response => response.json())
            .then((responseJson) => {
                console.log("responseJson", responseJson)
...
export default connect(mapStateToProps)(Clearbill_Claim);